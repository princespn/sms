...
class TTSController extends Controller
{
    ...
    {   
        ...
        $max_chars = (auth()->user()->group == 'user') ? config('tts.free_chars_limit') : config('tts.max_chars_limit');
        $vendor_logo['status'] = json_encode((config('tts.vendor_logos') == 'show') ? 'show' : 'none');

        $projects = Project::where('user_id', auth()->user()->id)->get();

        return view('user.tts.index', compact('languages', 'voices', 'max_chars', 'vendor_logo', 'projects'));
    }
