...
@section('page-header')
	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			<h4 class="page-title mb-0">{{ __('Synthesize Result Data') }}</h4>
			<ol class="breadcrumb mb-2">
				...
				<li class="breadcrumb-item active" aria-current="page"><a href="#"> {{ __('Synthesized Result Data') }}</a></li>
			</ol>
		</div>
	</div>
	<!-- END PAGE HEADER -->
		...
		<div class="col-lg-6 col-md-6 col-xm-12">
			<div class="card overflow-hidden border-0">
				<div class="card-header">
					<h3 class="card-title">{{ __('Text Synthesize Result') }}</h3>
				</div>
				<div class="card-body pt-5">

					<div class="row">
						<div class="col-lg-4 col-md-4 col-12">
							<h6 class="font-weight-bold mb-1">{{ __('Cloud Vendor') }}: </h6>
							<span class="fs-14">{{ strtoupper($id->vendor) }}</span>
						</div>
						<div class="col-lg-4 col-md-4 col-12">
							<h6 class="font-weight-bold mb-1">{{ __('Voice') }} ID: </h6>
							<span class="fs-14">{{ $id->voice_id }}</span>
						</div>
						<div class="col-lg-4 col-md-4 col-12">
					...
					<div class="row pt-4">
						...
						<div class="col-lg-4 col-md-4 col-12">
							<h6 class="font-weight-bold mb-1">{{ __('Estimated Cost') }}: </h6>
							<span class="fs-14">$<span id="estimated_value"></span></span>
						</div>
						<div class="col-lg-4 col-md-4 col-12">
							<h6 class="font-weight-bold mb-1">{{ __('Audio Storage') }}: </h6>
							<span class="fs-14">{{ ucfirst($id->storage) }}</span>
						</div>
					</div>		
					...
					<div class="row pt-7">
						<div class="col-12 mb-5">
							...
							<span class="fs-14">{{ $id->project }}</span>
						</div>
						<div class="col-12 mb-5">
					...
					<div class="row pt-7">
						<div class="col-12">
							<h6 class="font-weight-bold mb-3">{{ __('Synthesize Result') }}: </h6>
							<div id="user-result">																
								<div class="text-center user-result-player">
									<audio class="voice-audio">
										<source src="@if ($id->storage == 'local') {{ URL::asset($id->result_url) }} @else {{ $id->result_url }} @endif" type="{{ $id->audio_type }}">
									</audio>	
								</div>								
							</div>
						</div>
					...
					<div class="border-0 text-right mb-2 mt-8">
						<a href="{{ route('admin.tts.results') }}" class="btn btn-primary">{{ __('Return') }}</a>
					</div>
				</div>
			</div>
		</div>
...
@section('js')
	...
	<script type="text/javascript">
		$(function() {
			...
			var cost = JSON.parse(`<?php echo $json_data['cost']; ?>`);
			var costData = Object.entries(cost[0]);
			var value = costData[0][1].toFixed(5);

			document.getElementById('estimated_value').innerHTML = value;
