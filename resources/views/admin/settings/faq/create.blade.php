...
@section('page-header')
	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			<h4 class="page-title mb-0">{{ __('New FAQ Answer') }}</h4>
			<ol class="breadcrumb mb-2">
				...
				<li class="breadcrumb-item active" aria-current="page"><a href="{{url('#')}}"> {{ __('New FAQ') }}</a></li>
			</ol>
		</div>
		...
		<div class="col-lg-8 col-md-8 col-xm-12">
			<div class="card overflow-hidden border-0">
				<div class="card-header">
					<h3 class="card-title">{{ __('Create New FAQ Answer') }}</h3>
					<button type="button" class="btn btn-primary" id="create-category">Create New FAQ Category</button>				
				</div>
				<div class="card-body pt-5">									
					<form id="" action="{{ route('admin.settings.faq.store') }}" method="post" enctype="multipart/form-data">
						@csrf
							...
							<div class="col-lg-6 col-md-6 col-sm-12">							
								<div class="input-box">	
									<h6>{{ __('FAQ Status') }} <span class="text-muted">({{ __('Required') }})</span></h6>
			  						<select id="faq-status" name="status" data-placeholder="FAQ Status:">			
						...
						<div class="row mt-2">
							<div class="col-lg-12 col-md-12 col-sm-12">	
								<div class="input-box">	
									...
									<textarea class="form-control" name="answer" rows="12" id="richtext" required>{{ old('answer') }}</textarea>
									@error('answer')
