...
@section('page-header')
	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			<h4 class="page-title mb-0">{{ __('Synthesize Result Detail') }}</h4>
			<ol class="breadcrumb mb-2">
				<li class="breadcrumb-item"><a href="{{route('user.tts')}}"><i class="mdi mdi-audiobook mr-2 fs-12"></i>{{ __('User') }}</a></li>
				<li class="breadcrumb-item" aria-current="page"><a href="{{ route('user.tts.results') }}"> {{ __('My TTS Audio Results') }}</a></li>
				<li class="breadcrumb-item active" aria-current="page"><a href="{{ url('#') }}"> {{ __('Synthesize Result Detail') }}</a></li>
			</ol>
		</div>
	</div>
	<!-- END PAGE HEADER -->
			...
			<div class="card overflow-hidden border-0">
				...
				<div class="card-body pt-5">		
					...
					<div class="border-0 text-right mb-2 mt-8">
						<a href="{{ route('user.tts.results') }}" class="btn btn-primary">{{ __('Return') }}</a>
					</div>
				</div>
			</div>
		</div>
