	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			...
			<ol class="breadcrumb mb-2">
				...
				<li class="breadcrumb-item" aria-current="page"><a href="{{ route('user.referral') }}"> {{ __('My Referrals') }}</a></li>
			...
			<div class="card overflow-hidden border-0">
				...
				<div class="card-body">
					...
					<form action="{{ route('user.referral.payout.store') }}" method="POST" enctype="multipart/form-data">
						...
						<h6 class="fs-12 mb-6 mt-3">{{ __('Your preferred payout method is: ') }} <span class="font-weight-bold">@if (auth()->user()->referral_payment_method == '') Not Set <span class="text-muted">(Please configure it under My Gateway tab)</span> @else {{ auth()->user()->referral_payment_method }}@endif</span></h6>
