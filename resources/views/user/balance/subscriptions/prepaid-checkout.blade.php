...
@section('page-header')
	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			<h4 class="page-title mb-0">{{ __('Prepaid Plan Checkout') }}</h4>
		...
		<div class="col-md-8">
			<div class="card border-0 pt-2">
				<div class="card-body">	
					...
					<form id="payment-form" action="{{ route('user.payments.pay.prepaid', $id) }}" promocode="{{ route('user.payments.promocodes.prepaid', $id) }}" method="POST" enctype="multipart/form-data">
						@csrf
						...
						<div class="row">
															...
															<div class="col-md-4">															
																<label class="gateway btn rounded p-0" href="#{{ $payment_platform->name }}Collapse" data-toggle="collapse">
																	...
																	<img src="{{ URL::asset($payment_platform->image) }}" 
																		 class="@if ($payment_platform->name == 'Paystack' || $payment_platform->name == 'Razorpay') payment-image
							...
							<div class="col-md-6 col-sm-12 pl-4">
								<div class="checkout-wrapper-box">
									...
									<p class="checkout-title mt-2"><i class="fa fa-archive mr-2"></i>{{ __('Plan Name') }}: <span class="text-info">{{ $id->plan_name }}  {{ __('Prepaid Characters') }}</span></p>
									...
									<div>
										<p class="fs-12 p-family">{{ __('Subtotal') }} <span class="checkout-cost">{!! config('payment.default_system_currency_symbol') !!}{{ number_format((float)$id->cost, 2, '.', '') }}</span></p>
									...
									<div class="input-box mb-4">
										<div class="input-group">								
											<input type="text" class="form-control border-right-0 pt-4 pb-4" name="promo_code" placeholder="Promocode">
