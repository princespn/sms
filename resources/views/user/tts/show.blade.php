...
@section('page-header')
	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			<h4 class="page-title mb-0">{{ __('Synthesize Text Result') }}</h4>
			<ol class="breadcrumb">
				<li class="breadcrumb-item"><a href="{{route('user.tts')}}"><i class="fa fa-magic mr-2 fs-12"></i>{{ __('User') }}</a></li>
				<li class="breadcrumb-item" aria-current="page"><a href="{{ route('user.tts') }}"> Text-to-Speech</a></li>
				<li class="breadcrumb-item active" aria-current="page"><a href="{{url('#')}}"> {{ __('Synthesized Text Result') }}</a></li>
			</ol>
		</div>
	</div>
	<!-- END PAGE HEADER -->
			...
			<div class="card overflow-hidden border-0">
				...
				<div class="card-body pt-5">		
					...
					<div class="row pt-5">
						...
						@if (config('settings.vendor_logos') == 'show')
							<div class="col-lg-4 col-md-4 col-12">
						...
						<div class="col-lg-4 col-md-4 col-12 pt-5">
							...
							<span class="fs-14">@if ($id->result_ext == 'pcm') WAV @else {{ strtoupper($id->result_ext) }} @endif</span>						
						</div>
					</div>	
					...
					<div class="border-0 text-right mb-2 mt-8">
						<a href="{{ route('user.tts') }}" class="btn btn-primary">{{ __('Return') }}</a>
					</div>
				</div>
			</div>
		</div>
